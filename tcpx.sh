...
installbbr(){
	...
	if [[ "${release}" == "centos" ]]; then
		if [[ ${version} = "6" ]]; then
			...
			if [[ ${bit} = "x86_64" ]]; then
				wget -N -O kernel-headers-c7.rpm http://shell.kuaiyunds.com/shell/kernel-headers-c7.rpm
				wget -N -O kernel-c7.rpm http://shell.kuaiyunds.com/shell/kernel-c7.rpm
